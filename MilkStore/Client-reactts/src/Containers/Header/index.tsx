...
export const Header: React.FC<HeaderProps> = ({ dispatch }): JSX.Element => {
  ...
  let { current, milkCart, history, isSearching, searching } = state

  if(current === "/menu" && history === "/milk-cart"){
    history = "/"
  }
  ...
  const isCart: boolean = window.location.pathname === "/milk-cart";
      ...
      return(
        <React.Fragment>
          ...
          <nav className="Header__desktop">
            ...
            {window.location.pathname !== "/milk-cart" && (
              <button
                className="Header__cart"
                onClick={() => navigate("milk-cart")}
              >
                <span>Cart</span>
                <img src={Cart} alt="" />
                {(milkCart.length > 0) && (
